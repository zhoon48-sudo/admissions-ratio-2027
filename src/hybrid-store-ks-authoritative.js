import { collectHybrid } from './hybrid-collector-ks-authoritative.js';
...
export async function collectHybridAndStore(env, triggerType='manual'){
  ...
  try{
    ...
    `).bind(
      ...
      diagnosticText(r),r.url,r.sourceCollectedAt||collectedAt
    ));
    ...
    const authNames=Object.keys(data.ksAuthoritative?.targets||{});
    const note=`hybrid=uway-direct+jinhak-ks; authoritative=${authNames.join(',')||'none'}; roundId=${data.sourceRoundId??'unknown'}${delayedNames.length?`; delayed=${delayedNames.join(',')}`:''}`;

    await env.DB.prepare(
      `UPDATE crawl_runs SET finished_at=?, status=?, ok_count=?, error_count=?, note=? WHERE id=?`
    ).bind(finishedAt,errorCount===0?'success':'partial',okCount+delayedCount,errorCount,note,runId).run();

    let legacyImport=null;
    try{ legacyImport=await importLegacyReports(env); }
    catch(e){ legacyImport={ok:false,error:e instanceof Error?e.message:String(e)}; }

    return {
      saved:true,runId,startedAt,finishedAt,
      universities:(data.results||[]).length,
      okCount,delayedCount,errorCount,
      snapshotsSaved:(data.results||[]).length,
      sourceRoundId:data.sourceRoundId??null,
      collectionMode:'ks-authoritative',
      authoritativeUniversities:authNames,
      delayedUniversities:delayedNames,
      ksAuthoritative:data.ksAuthoritative||null,
      legacyImport
    };
  }catch(e){
    const finishedAt=new Date().toISOString();
